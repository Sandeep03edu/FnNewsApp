...
const InfiniteNews = (props)=> {
  ...
  // document.title = `${capitalizeWord(props.category)} Newz`;

  const capitalizeWord = (string) => {
    return string[0].toUpperCase() + string.slice(1);
  };

  // We are using useEffect to be called whenever the [] -> [input] function changes its data
  // Since it is an [] so it will trigger only on the first go
  useEffect(() => {
    updateNews(1);
  }, [])
  ...
  // render() {
    return (
      <>
        <h2>{capitalizeWord(props.category)} - Top Headlines</h2>
        {loading && <Spinner />}
        <InfiniteScroll
...
InfiniteNews.defaultProps = {
  country: "in",
  category: "general",
  pageSize: 10,
};

InfiniteNews.propTypes = {
